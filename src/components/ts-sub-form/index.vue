<template>
  <div style="padding: 20px 0;margin-bottom: 30px">
    ...
    >
     ...
     border-radius: 8px;position: relative;padding-top: 20px;margin-top: 10px;"
      >
        <div style="cursor:pointer;width: 20px;position: absolute;left: 4px;top: 4px;
          height: 20px;text-align: center;background: #30303080;border-radius: 50%;line-height: 20px;color: white;"
             @mouseenter="onMouseOver" @mouseleave="onMouseOut($event, I + 1)" @click="ts_delete_rows(I)"
        >
          {{ I + 1 }}
        </div>
    ...
    <div style="margin-top: 10px">
      <el-button v-if="canEdit && addButton && tableData.length > 0" size="small" icon="el-icon-plus" style="float: left;"
                 @click="ts_add_rows"
      >
        添加
      </el-button>
